...
class MessageTemplateResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Select::make('type')->label('Uso')->options(['confirmation' => 'Confirmação', 'recall' => 'Recall', 'reactivation' => 'Reativação', 'follow_up' => 'Follow-up'])->required(),
            Textarea::make('body')->label('Mensagem')->required()->rows(7)->helperText('Variáveis: {{cliente}}, {{empresa}}, {{servico}}, {{data}}, {{horario}}, {{link_agendamento}}.')->columnSpanFull(),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('type')->label('Uso')->badge(),
            IconColumn::make('active')->label('Ativo')->boolean(),
            TextColumn::make('updated_at')->label('Atualizado')->dateTime('d/m/Y H:i'),
        ])->recordActions([EditAction::make()->url(fn (MessageTemplate $record) => self::getUrl('edit', ['record' => $record])), DeleteAction::make()])->toolbarActions([BulkActionGroup::make([DeleteBulkAction::make()])]);
    }
