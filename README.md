...
SaaS para pet shops de pequeno porte organizarem agenda, pets, responsáveis, pacotes e confirmações. O envio por WhatsApp é manual nesta versão: o painel abre uma conversa `wa.me` e o atendente registra o resultado.
...
O painel da empresa fica em `/admin` e o painel reservado a superadmins fica em `/platform`. Para promover uma conta existente, atualize `users.is_super_admin` para `true` diretamente no banco durante a fase inicial.
